export type CardImageGapStatus = "planned";

export type CardImageGapPriority = "P0" | "P1";

export type CardImageGapType = "basic" | "tactic" | "equipment";

export interface CardImageGapAsset {
  cardId: string;
  name: string;
  type: CardImageGapType;
  priority: CardImageGapPriority;
  aspectRatio: "3:4";
  path: string;
  promptZh: string;
  promptEn: string;
  negativePrompt: string;
  status: CardImageGapStatus;
}

const negativePrompt = "文字、Logo、水印、UI 邊框、現代武器、科幻裝備、血腥畫面、畸形手指、重複物件、低清晰度";

export const CARD_IMAGE_GAP_MANIFEST: CardImageGapAsset[] = [
  {
    cardId: "slash",
    name: "斬",
    type: "basic",
